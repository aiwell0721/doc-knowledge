...
### 2.2 图像级默认 prompt（数字识别约束，v0.3.0）

cloud/local/hybrid 三种图像级模式共用 `LLMVisionService`（`src/doc_knowledge/vision.py`），其默认 system_prompt 控制 VLM 识别行为：
...
只输出识别到的文字，不要添加额外评论。
识别数字时必须依据视觉形状特征（封闭圆圈数、开口方向、弧线走向）判断，禁止臆测；
对无法确认的字符标注 [?可能为X]。
```

**v0.3.0 增强（2026-08-14）**：借鉴 skillhub `pdf-image-text-extractor` 的提示词工程，追加两条数字识别约束——

- **形状特征优先**：数字判定以视觉形状（封闭圆圈数 / 开口方向 / 弧线走向）为准，禁止凭上下文臆测；
- **不确定标注**：无法确认的字符用 `[?可能为X]` 标注，提示用户复核而非静默输出错误字符。

**slide 模式不受影响**——使用独立 `DEFAULT_SLIDE_PROMPT`（§7.4），双契约接口互不污染（§7.3）。默认值仅在 `system_prompt=None` 时生效，显式传入自定义 prompt 仍以自定义为准。
...
③ 整页 PNG 批量送云端 VLM                             →  每页约定标记 Markdown
       （[DK-标题]/[DK-概述]/[DK-结构]/[DK-正文]，见 §7.4）
...
**产物形态（2026-08-09 二次变更，结构化输出；2026-08-10 三次变更，JSON→约定标记 Markdown）**：每页注入"`📊 标题（结构类型）` + 概述 + 按识别结构组织的正文"，末尾以 `<details>` **折叠 MarkItDown 原文**作兜底参考（可靠性 > 体积，默认保留）。原 `> 📊 **整页理解**: {JSON}` blockquote 形态**废弃**——裸 JSON 可读性差、`charts/tables` 字段实测大量为空（10/10 空），结构化价值名存实亡。**2026-08-10 起 VLM 输出由结构化 JSON 迁移为约定标记 Markdown（`[DK-xxx]`）**，彻底根治 JSON 生成脆弱性（GLM 实测 89% 非法），见 §7.4。
...
**失败判定（统一，2026-08-10 更新）**：`成功 = text.strip() 非空 且 不以 "[图片识别失败" / "[图片解析失败" 开头`。不再用"以 `[` 开头即失败"——约定标记 Markdown（`[DK-标题]` 等）也以 `[` 开头，精确匹配错误前缀避免把正常标记输出误判为失败而无限重试。空串、纯空白均视为失败。

**失败块兼容形态（2026-08-09 二次更新；2026-08-10 补充）**：`retry-slide` 解析失败页清单与 `_update_slide_blockquotes` 删除旧块时，需匹配以下失败形态——
- 结构化形态降级提示：`⚠️ 本页图表语义识别失败（VLM 请求异常），原始文字已保留`
- 缺 body / 未守标记格式降级：`> 📊 **整页理解**: {原文}`（VLM 返回文本但缺 `[DK-正文]`，或仍输出 JSON 时的兜底；原文直接展示，不折叠）
...
### 7.4 默认 prompt（slide 模式专用，2026-08-10 迁移为约定标记 Markdown）

**设计原则**：VLM 输出即决定产物本质，prompt 直接要求"内容逻辑结构识别 + 按结构组织正文"。原 `page_summary/charts/tables/layout_notes` 4 字段**废弃**（实测 charts 10/10 空、tables 8/10 空，结构化名存实亡）。

**迁移决策（2026-08-10，JSON → 约定标记 Markdown）**：GLM-4.6V-Flash 生成结构化 JSON 极不可靠——实测 89% 非法（body 内真实换行/制表符等未转义控制字符，修复后仍 20% 因未转义英文引号降级）。JSON 生成对免费 flash 模型是**能力边界问题**，不是解析层容错（```json 围栏剥离 + `strict=False` 宽松解析）能根本解决的。改用纯文本标记 Markdown：

- 纯文本生成，**无转义负担**（不再出现字符串内未转义引号/控制字符导致整页降级）；
- 保留结构化字段（title/overview/structure）可正则提取；
- 产物本身就是可读 Markdown，无需二次渲染；
- 代码简化（删除 `json.loads` 容错层，见解析规则）。

**双模型验证（2026-08-10）**：对 10 页样例 PPT，GLM-4.6V-Flash 与 Qwen2.5-VL-7B 均实现 **100% 标记合规、0 降级**（对比 JSON 时代 GLM 89% 非法/20% 降级）。GLM 完整输出 4 标记，Qwen 仅输出必需标记（标题+正文）但优雅降级。详见 `output/reports/dk-vlm-compare-v2-2026-08-10.md`。

**prompt 强化学（2026-08-10）**：初测 Qwen2.5-VL-7B 概述/结构提取成功率 0% —— 7B 小模型对"可省略"理解为直接跳过标签（输出满足约束的最短路径）。将 `[DK-概述]`/`[DK-结构]` 改为"必须输出"后重验：概述/结构恢复输出，但 **`[DK-标题]` 仍被省略**（"标题可省略"→Qwen 连标题也跳过，内容并入概述）——故四标记（标题/概述/结构/正文）全部改为强制，不留任何"可省略"。渲染层不引入"无"占位符，避免出现 `（无）`/`**概述**：无` 等劣化产物。
...
```markdown
[DK-标题]          ← 必须（prompt 强制；解析层仍容忍缺失）
市场规模预测

[DK-概述]          ← 必须（prompt 强制；解析层仍容忍缺失）
2026年 AI 市场预计超过700亿元

[DK-结构]          ← 必须（prompt 强制；解析层仍容忍缺失）
总分结构
...
- **`[DK-正文]` 之后到文本末尾的全部内容 = body**，仅 body 为必需字段（缺或空 → `_parse_slide_result` 返回 None，走降级）；
- 标题/概述/结构由 prompt 强制必须输出（2026-08-10 补强，见下"prompt 强化学"），解析层仍容忍缺失（宽松兜底：标签存在但值为空 → 跳过，渲染层已处理空值）。

**解析规则（`converters._parse_slide_result`，2026-08-10）**：正则提取各 `[DK-xxx]` 标签值；标签值 = 标签后到下一个 `[DK-` 标签（或文本末尾）前的全部内容；`[DK-正文]` 特殊——取到文本末尾，故 body 可含任意 Markdown 不被后续标签截断。错误占位符（`[图片识别失败...]` / `[图片解析失败...]`）直接返回 None。
...
注：不用 blockquote 包整个 body（表格/列表会被 `>` 前缀破坏渲染）；原文 `<details>` 默认收起，作 VLM 复述的准确性校验源。渲染层 `_render_slide_page` 接口不变（仍接收解析后 dict），其对非 str 字段的防御性转换（dict → `###` 小标题、list → 项目符号）保留为兜底路径（标记迁移后字段恒为 str，该路径不再被触发）。

**JSON → 标记迁移对解析层的影响（2026-08-10）**：原 JSON 解析容错层（```json 围栏剥离 + `json.loads(text, strict=False)` 宽松解析，2026-08-10 早期修复）随迁移**整体删除**——纯文本标记无转义问题，不再需要容错。`_is_success` 同步由"不以 `[` 开头"改为精确匹配 `[图片识别失败` / `[图片解析失败` 前缀，排除 `[DK-` 标记被误判为失败。
